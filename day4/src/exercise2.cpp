...
// 頂点オブジェクト
struct Vertex {
    Vertex(const glm::vec3 &position_, const glm::vec2 &texcoord_, const float &faceID_)
    : position(position_)
    , texcoord(texcoord_)
    , faceID(faceID_){
    }
...
void initVAO() {
    ...
    for (int i = 0; i < 6; i++) {
        for (int j = 0; j < 3; j++) {
            vertices.push_back(Vertex(positions[faces[i * 2 + 0][j]], texcoords[texIndices[i * 2 + 0][j]], float(i + 1)));
            indices.push_back(idx++);
        }
        
        for (int j = 0; j < 3; j++) {
            vertices.push_back(Vertex(positions[faces[i * 2 + 1][j]], texcoords[texIndices[i * 2 + 1][j]], float(i + 1)));
            indices.push_back(idx++);
        }
    }
    ...
    glVertexAttribPointer(2, 1, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)offsetof(Vertex, faceID));   //floatで渡す
...
void paintGL() {
    ...
    // 1つ目の立方体を描画
    {
        ...
        modelMat = glm::rotate(modelMat, theta, glm::vec3(rotateX, rotateY, 0.0f));
        ...
        glUniform1f(uid, float(selectID));
